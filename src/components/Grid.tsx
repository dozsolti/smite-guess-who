...
import { useGod, useFilters } from "../store";
...
export default function Grid() {
  ...
  return (
    <div className="gap-1.5 md:gap-2 grid grid-cols-4 md:grid-cols-7 lg:grid-cols-8 xl:grid-cols-11 mx-auto mt-2 md:mt-0 px-4 xl:px-0 cursor-pointer container">
      <div className="col-span-full" />
      {gods.map((god) => (
        <div
          key={god.name}
          className={isGodHidden(god) ? "opacity-10 is-hidden" : ""}
        >
          <GodCard
            god={god}
            onClick={() => {
